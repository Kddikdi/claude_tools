...
ふるさと納税の上限額シミュレーターや手取り計算シミュレーターなど、お金にまつわる無料ツールをまとめた静的サイトです。共通のヘッダー・フッター・ナビゲーションを持ち、各ツールは `/tools/<ツール名>/` のサブディレクトリに配置されています。
...
- [ ] `src/pages/privacy.html`・`about.html`・`contact.html` 内の `<!-- -->` コメント箇所（運営者名、連絡先メールアドレス、制定日など）を実際の情報に差し替える
